...
function wrappedCallback(callback) {
    function dataCallback(data, original) {
        let normalizedData = callback(data) || data;
        if (original) {
...
function uuid4() {
    let crypto = _window.crypto || _window.msCrypto;
...
function isSameStacktrace(stack1, stack2) {
    ...
    let frames1 = stack1.frames;
    let frames2 = stack2.frames;
...
let MAX_SERIALIZE_EXCEPTION_DEPTH = 3;
// 50kB, as 100kB is max payload size, so half sounds reasonable
let MAX_SERIALIZE_EXCEPTION_SIZE = 50 * 1024;
let MAX_SERIALIZE_KEYS_LENGTH = 40;
...
function serializeObject(value, depth) {
    ...
    if (isPlainObject(value)) {
        ...
        return value.map(val => serializeObject(val, depth - 1));
    }
...
function serializeException(ex, depth, maxSize) {
    ...
    let serialized = serializeObject(ex, depth);
...
function sanitize(input, sanitizeKeys) {
    ...
    let sanitizeRegExp = joinRegExp(sanitizeKeys);
    let sanitizeMask = '********';
    let safeInput;
    ...
    function sanitizeWorker(workerInput) {
        if (isArray(workerInput)) {
            return workerInput.map(val => sanitizeWorker(val));
        }

        if (isPlainObject(workerInput)) {
            return Object.keys(workerInput).reduce((acc, k) => {
